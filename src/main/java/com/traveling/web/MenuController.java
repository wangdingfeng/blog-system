...
public class MenuController {
    ...
    */
    ...
    public Map<String, Object> selectMenuById(Integer id) {
        ...
        try {
            if(id==null||id==0){
                ...
                SmpMenu tmenu=tmenuService.selectByKey(id);
                if(tmenu==null){
                    resultmap.put("state", "fail");
    ...
    @RequiresPermissions(value = {"菜单管理"})
    public Map<String, Object> addupdatemenu(HttpSession session, SmpMenu tmenu) {
        LinkedHashMap<String, Object> resultmap = new LinkedHashMap<String, Object>();
        try {
            if (tmenu.getId() == null) {//新建
                ...
                if(tmenu.getpId()==null||tmenu.getpId()==0){
                    ...
                    SmpMenu pmenu=tmenuService.selectByKey(tmenu.getpId());//父节点对象
                    if(pmenu.getState()==3){
                        resultmap.put("state", "fail");
                    ...
                    List<SmpMenu> list=tmenuService.selectByExample(example);
    ...
    @RequiresPermissions(value = {"菜单管理"})
    public Map<String, Object> deletemenu(HttpSession session,SmpMenu tmenu) {
        LinkedHashMap<String, Object> resultmap = new LinkedHashMap<String, Object>();
        try {
            if(tmenu.getId()!=null&&!tmenu.getId().equals(0)){
                SmpMenu menu=tmenuService.selectByKey(tmenu.getId());
                if(menu==null){
                    resultmap.put("state", "fail");
