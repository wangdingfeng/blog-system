...
public class RoleAdminController {
    ...
    @Resource
    private SmpRoleMenuService trolemenuService;
    ...
    public Map<String, Object> list(JqgridBean jqgridbean
        ...
        List<SmpRole> roleList = roleService.selectByExample(troleExample);
        PageRusult<SmpRole> pageRusult =new PageRusult<SmpRole>(roleList);

        /*Integer totalrecords = roleService.countByExample(SmpRoleExample);//总记录数
    ...
    @RequiresPermissions(value = {"角色管理"})
    public Map<String, Object> addupdaterole(SmpRole trole) {
        LinkedHashMap<String, Object> resultmap = new LinkedHashMap<String, Object>();
        try {
            if (trole.getId() == null) {//新建
                ...
                SmpRole oldObject = roleService.selectByKey(trole.getId());
                if (oldObject == null) {
                    resultmap.put("state", "fail");
    ...
    @RequiresPermissions(value = {"角色管理"})
    public Map<String, Object> deleteuser(SmpRole trole) {
        LinkedHashMap<String, Object> resultmap = new LinkedHashMap<String, Object>();
        try {
            if (trole.getId() != null && !trole.getId().equals(0)) {
                SmpRole role = roleService.selectByKey(trole.getId());
                if (role == null) {
                    resultmap.put("state", "fail");
                    ...
                    Example trolemenuexample = new Example(SmpRoleMenu.class);
    ...
    @RequiresPermissions(value = {"角色管理"})
    public Map<String, Object> selectRoleById(SmpRole trole) {
        LinkedHashMap<String, Object> resultmap = new LinkedHashMap<String, Object>();
        try {
            if (trole.getId() != null && !trole.getId().equals(0)) {
    ...
    public String loadCheckMenuInfo(Integer parentId, Integer roleId) throws Exception {
        List<SmpMenu> menuList = tmenuService.selectMenusByRoleId(roleId);// 根据角色查询所有权限菜单信息
        //移除所有没有pid的menuid
        Iterator<SmpMenu> it = menuList.iterator();
        while (it.hasNext()) {
            SmpMenu tmenu = it.next();
    ...
    private JsonArray getCheckedMenuByParentId(Integer parentId, List<Integer> menuIdList) {
        Example tmenuExample = new Example(SmpMenu.class);
        tmenuExample.or().andEqualTo("pId",parentId);
        List<SmpMenu> menuList = tmenuService.selectByExample(tmenuExample);
    ...
    public Map<String, Object> saveMenuSet(String menuIds, Integer roleId) throws Exception {
        ...
        if (StringUtils.isNotEmpty(menuIds)) {
            ...
            List<SmpMenu> menuList = tmenuService.selectMenusByRoleId(roleId);
            //移除所有没有pid的menuid
            Iterator<SmpMenu> it = menuList.iterator();
            while (it.hasNext()) {
                SmpMenu tmenu = it.next();
            ...
            for (int i = 0; i < idsStr.length; i++) { // 然后添加所有角色权限关联实体
                SmpRoleMenu trolemenu = new SmpRoleMenu();
    ...
    public Map<String, Object> saveColumnsSet(String permisIds, Integer roleId) throws Exception {
        ...
        if (StringUtils.isNotEmpty(permisIds.trim())) {
            ...
            for (int i = 0; i < namesStr.length; i++) { // 然后添加所有角色权限关联实体
                ...
                List<SmpMenu> tmenuList = tmenuService.selectByExample(tmenuExample);
                if (tmenuList == null || tmenuList.size() == 0) {
                    //当前menu表还没有这个name的记录，则先新增menu表的记录，再新增中间表的几率
                    SmpMenu tmenu = new SmpMenu();
                    tmenu.setName(namesStr[i]);
                    tmenuService.saveNotNull(tmenu);

                    Example tmenuExample2 = new Example(SmpMenu.class);
                    tmenuExample2.or().andEqualTo("name",tmenu.getName()).andIsNull("pId");
                    List<SmpMenu> tmenuList2 = tmenuService.selectByExample(tmenuExample2);
