...
public class UserController {
	...
	@Resource
	private SmpRoleService troleService;
	
	@Resource
	private SmpUserService tuserService;
    ...
    @ResponseBody
    @PostMapping("/login")
    public Map<String,Object> login(String imageCode, @Valid SmpUser user, BindingResult bindingResult, HttpSession session){
...
/*    	if(StringUtils.isEmpty(imageCode)){
		...
		try{
			...
			//SmpUser currentUser=tuserService.findByUserName(userName);
			Example tuserExample=new Example(SmpUser.class);
			tuserExample.or().andEqualTo("userName",userName);
			SmpUser currentUser=tuserService.selectByExample(tuserExample).get(0);
			session.setAttribute("currentUser", currentUser);
			//List<SmpRole> roleList=troleService.findByUserId(currentUser.getId());
			List<SmpRole> roleList=troleService.selectRolesByUserId(currentUser.getId());
	...
	public Map<String,Object> saveRole(Integer roleId,HttpSession session)throws Exception{
		Map<String,Object> map=new HashMap<String,Object>();
		SmpRole currentRole=troleService.selectByKey(roleId);
